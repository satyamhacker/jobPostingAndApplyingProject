...
const OrganizationResetPassword = () => {
  ...
  const roles = ['HR', 'Admin', 'Manager']; // Define available roles
  ...
  const handleResetPassword = async () => {
    if (!newPassword || !role) {
      setError('New password and role are required');
      return;
    }
    try {
      const response = await fetch('http://localhost:3000/organization-reset-password', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
